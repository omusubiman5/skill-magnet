...
Windows版MVPの画面は、起動後にマウスを使わず操作できます。Tab／Shift+Tabで項目を移動し、一覧は矢印キーで選択、ボタンはEnterまたはSpaceで実行、Escで破壊的処理を確定せず安全に閉じます。読み取り専用の変更内容表示はTab移動の対象外です。受入条件は [キーボード操作テスト計画](docs/windows-mvp-keyboard-test-plan-2026-09-06.md) に記録しています。
