...
右クリックしたフォルダーは、スキルのインストール先ではなく、依頼を実行して成果物を置くための「作業対象フォルダー」としてデスクトップアプリへ渡します。`~/.codex/skills`、`~/.agents/skills`、`~/.claude/skills`とその配下は各ランタイムのスキル管理領域なので、作業対象には指定できません。これらを右クリックして実行した場合は、契約作成、スキルのコピー、デスクトップアプリへのhandoffを行う前にエラーで停止します。
