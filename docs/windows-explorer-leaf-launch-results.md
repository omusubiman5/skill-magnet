...
- 0.5.4ではLibrary Managerへpack→skill一覧とCRUDを追加し、管理対象ファイルだけの削除公開、依存削除防止、隔離候補rollback、同一repository旧packの有効設定除去、GitHub差分0件時の再同期を実装した。release candidate wheelの論理payload SHA-256は`ce651401d6c84ad174633f31f98d249bad89b13cfce0bc79893d71e034749f1e`である。release codeは`f54b865dbbcae2625756c814c641942db18cb70d`である。
